class LookupDrugs
  ...
  def self.match_search_terms_to_objects(search_terms, scope, filter)
    ...
    drug_results = filter.filter_rel(DataModel::Drug.send(scope).where(["name in (?) or chembl_id in (?)", search_terms, search_terms]))
    search_terms = search_terms - drug_results.map(&:name) - drug_results.map(&:chembl_id)
    drug_alias_results = filter.filter_rel(DataModel::DrugAlias.send(scope).where(alias: search_terms))
