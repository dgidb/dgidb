class LookupGenes
  ...
  def self.match_search_terms_to_objects(search_terms, scope, filter)
    ...
    gene_results = filter.filter_rel(DataModel::Gene.send(scope).where(["genes.name in (?) or entrez_id in (?)", search_terms, search_terms.map {|x| Integer(x) rescue nil }.compact]))
