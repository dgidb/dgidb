module DataModel
  class Interaction < ::ActiveRecord::Base
    ...
    belongs_to :drug, foreign_key: :drug_name_report_id, inverse_of: :interactions
    has_many :interaction_attributes, inverse_of: :interaction
    belongs_to :citation, inverse_of: :interactions

    def types
      self.interaction_attributes.find_all{|attribute| attribute.name == 'Interaction Type'}
    end
    ...
    def self.basic
      ...
      memoize_query(DataModel::Interaction.joins{drug}.where{
        id.not_in(my{attribute("Interaction Type", "potentiator")}) &
        ( id.not_in(my{attribute("Interaction Type", "na")}) | id.in( my{attribute("is_known_action", "yes" )}) ) &
    ...
    def self.inhibitors_only
      ...
      memoize_query(DataModel::Interaction.joins{drug}.where{
        id.not_in(my{attribute("Interaction Type", "potentiator")}) &
        id.in(my{attribute("Interaction Type", "inhibitor")}) &
        ( id.not_in(my{attribute("Interaction Type", "na")}) | id.in( my{attribute("is_known_action", "yes" )}) ) &
        drug.id.not_in(my{drug_category("withdrawn","nutraceutical")})
